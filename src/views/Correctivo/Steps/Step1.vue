<template>
  <div class="flex w-[100%]">
    <div class="mb-3 flex w-full lg:w-[90%] flex-col items-start justify-start lg:pl-2">
      ...
      <TransitionRoot :show="solicitud" enter="transition-opacity duration-75" enter-from="opacity-0"
        enter-to="opacity-100" leave="transition-opacity duration-150" leave-from="opacity-100" leave-to="opacity-0">
        <div role="status" class="flex h-52 flex-col items-center justify-center">
          <svg aria-hidden="true" class="mr-2 h-8 w-8 animate-spin fill-blue-800 text-gray-200 dark:text-gray-600"
      ...
      <div v-if="error" class="mt-2 flex w-[100%] items-center justify-center py-1" :class="rebotar">
    ...
    <div class="flex h-full items-center">
      <ChevronRightIcon @click="cambiarEstado()"
...
const emits = defineEmits(["capturarFolio"])
...
const cambiarEstado = async () => {
  const result = await get(refDB(db, `folios/correctivos/${props.folio}`));
