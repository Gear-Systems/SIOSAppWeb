<template>
  <div>
    ...
    <TransitionRoot appear :show="$store.state.a.modalPreventivo" as="template" class="z-50">
          ...
          <div class="flex min-h-full items-center justify-center p-4 text-center">
            <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
              ...
              <DialogPanel
                ...
                <DialogTitle as="div" class="mb-5 flex items-center text-lg font-medium leading-6 text-gray-900">
                  <div class="flex w-[80%] text-2xl font-bold">
                    <h3>Folio Preventivo</h3>
                  </div>
                  <div class="flex w-[20%] justify-end">
                    <XCircleIcon @click="closeModal" class="h-6 w-6 cursor-pointer" />
                  </div>
                </DialogTitle>
                <div class="flex w-[100%] flex-col items-center justify-center">
                  <div class="my-2 flex w-[100%] items-center justify-center">
                    <div class="flex w-2/3 flex-col text-xs font-normal text-[#C4C4C4]">
                      Ingresar folio
                      <input v-model="infoSelected.folio" placeholder="000" :class="[
                    ...
                    <div class="flex w-1/3 flex-col text-xs font-normal text-[#C4C4C4]">
                      Tipo
                      <div class="flex w-[100%]">
                        <ListSelect :dataArray="infoData.tipoFolios" :label="'Tipo de folio'"
                          @inputValue="validarTipoFolio($event)" />
                      </div>
                    </div>
                  </div>
                  <div class="mb-4 flex w-[100%] items-center justify-between">
                    <div class="mr-2 flex w-1/2 flex-col text-xs font-normal text-[#C4C4C4]">
                      Distrito
                      <div class="flex w-[100%]">
                        <ListSelect :dataArray="infoData.distritos" :label="'Selecciona un distrito'"
                    ...
                    <div class="ml-2 flex w-1/2 flex-col text-xs font-normal text-[#C4C4C4]">
                      Cluster
                      <div class="flex w-[100%]">
                        <ListSelect :dataArray="
                          infoSelected.distrito
                            ? infoSelected.distrito.clusters
                            : []
                        " :label="'Selecciona un cluster'" @inputValue="infoSelected.clusters = $event" />
                      </div>
                    </div>
                  </div>
                  <div class="mb-4 flex w-[100%] items-center justify-between">
                      ...
                      <div class="flex w-[100%]">
                        <ListSelect :dataArray="
                          infoSelected.distrito
                            ...
                            : []
                        " :label="'Selecciona un supervisor'" @inputValue="obtenerTecnicos($event)" />
                      </div>
                    </div>
                    <div class="ml-2 flex w-1/2 flex-col text-xs font-normal text-[#C4C4C4]">
                      Técnico
                      <div class="flex w-[100%]">
                        <ListSelect :dataArray="infoData.tecnicos" :label="'Selecciona un técnico'"
                  ...
                  <div class="mb-4 flex w-[80%] items-center self-start">
                    <div class="mr-2 flex w-1/2 flex-col text-xs font-normal text-[#C4C4C4]">
                      Ingresar OLT
                      <input v-model="infoSelected.olt" placeholder="000"
                        class="mt-1 h-10 w-[100%] rounded-lg border-2 border-[#E5E5E5] text-black" type="text"
                        name="folio" id="folio" />
                      <Transition leave-active-class="transition duration-200 ease-in" leave-from-class="opacity-100"
                        leave-to-class="opacity-0" enter-active-class="transition duration-200 ease-in"
                        enter-from-class="opacity-0" enter-to-class="opacity-100">
                    ...
                    <div class="ml-2 flex w-1/2 flex-col text-xs font-normal text-[#C4C4C4]">
                      Falla
                      <div class="flex w-[100%]">
                        <ListSelect :dataArray="infoData.fallas" :label="'Selecciona una falla'"
                          @inputValue="infoSelected.falla = $event" />
                      </div>
                    </div>
                  </div>
                  <div class="mb-4 flex w-[80%] items-center self-start">
                    <div class="mr-1.5 flex w-1/2 flex-col text-xs font-normal text-[#C4C4C4]">
                      Causa
                      <div class="flex w-[100%]">
                        <ListSelect :dataArray="infoData.causas" :label="'Selecciona una causa'"
                          @inputValue="infoSelected.causa = $event" />
                      </div>
                    </div>
                    <div class="ml-2 flex w-1/2 flex-col text-xs font-normal text-[#C4C4C4]">
                      Turno
                      <ListSelect :dataArray="infoData.turnos" :label="'Selecciona un turno'"
                ...
                <div class="mt-4 flex justify-around">
                  ...
                  <div class="flex w-1/2 justify-center">
                    <button :disabled="buttonDisabled || loadingSubmit" type="button" :class="[
                    ...
                    ]" @click="submitPreventivo">
                      <svg v-if="loadingSubmit" aria-hidden="true"
    ...
    <ModalManejoFolio :botonCapturar="true" :permitirCierre="false" @asignarFolio="asignarFolio"
      @capturarFolio="capturarFolio" :infoSelected="infoSelected">
  ...
  get(catalogoRef).then((snapshot) => {
    snapshot.forEach((element) => {
      ...
      if (element.key === "tipoFolios") {
        for (let data in element.val().preventivo) {
          infoData.tipoFolios.push({
            name: data,
            ...element.val().preventivo[data],
          });
        }
      }
      // distritos
