<template>
    <div class="relative flex h-full w-full flex-col overflow-hidden p-8">
      ...
      <div class="mt-2 flex h-full flex-row p-4">
        ...
        <div class="flex max-h-[380px] w-full overflow-y-auto overflow-x-hidden">
          <MantenimientoPreventivoPaso1 v-if="!loading && paso == 1" />
          <MantenimientoPreventivoPaso2 v-if="!loading && paso == 2" />
          <Suspense>
            <MantenimientoPreventivoPaso3 v-if="!loading && paso == 3" />
          </Suspense>
        </div>
      </div>
      <!-- timeline -->
  ...
  import MantenimientoPreventivoPaso1 from "@/components/MantenimientoPreventivoPaso1.vue";
  import MantenimientoPreventivoPaso2 from "@/components/MantenimientoPreventivoPaso2.vue";
  import { storeToRefs } from "pinia";
  import MantenimientoPreventivoPaso3 from "@/components/MantenimientoPreventivoPaso3.vue";
  ...
  const correctivoRef = refDB(db, `folios/preventivos/${route.params.id}`);
  
  onBeforeMount(async () => {
    await fetchData(route.params.id, "preventivos").then(() => {
      loading.value = false;
  ...
  onChildChanged(correctivoRef, (snapshot) => {
    fetchData(route.params.id, "preventivos");
  });
  </script>
