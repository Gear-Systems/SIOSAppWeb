<template>
  <div class="flex w-full">
    <div class="flex w-full select-none flex-col space-y-3 lg:w-[90%]">
      <div
                  ...
                  <div class="mt-8 flex justify-around">
                    ...
                    >
                      ...
                      <button
                        ...
                        @click="crearNota(mensaje, color.color, color.lateral)"
                      >
                        Guardar
                      </button>
                      <button
...
import { ref as refDB, set, update, remove, push } from "firebase/database";

const foliosController = useFolios();
const { foliosData } = storeToRefs(foliosController);
...
// Crear nueva nota
const crearNota = async (msg, color_nuevo, lateral_nuevo) => {
  console.log([msg, color_nuevo, lateral_nuevo]);
  if (!msg.trim() == "") {
    const notasRef = refDB(
      db,
      `folios/${foliosData.value.incidencia}/${foliosData.value.folioKey}/notas`
    );
    const nuevaNotaRef = push(notasRef);
    await set(nuevaNotaRef, {
      texto: msg,
      color: color_nuevo,
      colorLateral: lateral_nuevo,
    });
    notas.value.push({
      llave: nuevaNotaRef.key,
      texto: msg,
      color: color_nuevo,
...
//  Eliminar nota
const eliminarNota = async (notaKey) => {
  await remove(
    refDB(
      db,
      `folios/${foliosData.value.incidencia}/${foliosData.value.folioKey}/notas/${notaKey}`
    )
  ).then(() => {
    let index = notas.value.findIndex((nota) => nota.llave == notaKey);
...
// actualizar nota
const actualizarNota = async (msg, color_nuevo, lateral_nuevo, notaKey) => {
  if (msg.trim() != "") {
    await update(
      refDB(
        db,
      ...
      let index = notas.value.findIndex((nota) => nota.llave == notaKey);
      if (index != -1) {
        Object.assign(notas.value[index], {
          llave: notaKey,
          texto: msg,
          color: color_nuevo,
          color_lateral: lateral_nuevo,
...
onBeforeMount(() => {
  if (foliosData.value.notas) {
    for (let nota in foliosData.value.notas) {
      notas.value.push({
        llave: nota,
        texto: foliosData.value.notas[nota].texto,
        color_lateral: foliosData.value.notas[nota].colorLateral,
        color: foliosData.value.notas[nota].color,
      });
    }
  }
});
