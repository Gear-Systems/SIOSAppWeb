...
const actualizar = () => {
  if (newName) {
    get(
      child(
        refDB(db),
        `catalogo/${props.data.tipo.toLowerCase()}/${props.data.nombre}`
      )
    ).then((snapshot) => {
      update(
        refDB(db, `catalogo/${props.data.tipo.toLowerCase()}/${newName.value}`), snapshot.val())
            .then((snapshot) => {
                remove(refDB(db, `catalogo/${props.data.tipo.toLowerCase()}/${props.data.nombre}`));
                emit("closeModal");
            });
    });
  }
