    ...
    <transition enter-active-class="transition duration-100 ease-out" enter-from-class="transform scale-95 opacity-0"
      ...
      <MenuItems
        ...
        <div class="px-1 py-1">
          ...
          <button @click="desasignarFolio()" :class="[
            active ? 'bg-[#E9F0FC] text-primario' : 'text-black',
            'group flex w-full items-center rounded-md px-2 py-2 text-sm',
          ]">
            <DesasignarFolioIco class="mr-2 h-5 w-5 text-violet-400" aria-hidden="true"
              :color="active ? '#2166E5' : '#101010'" />
            Desasignar folio
          </button>
          </MenuItem>
          <MenuItem v-slot="{ active }">
          <button @click="eliminarFolio()" :class="[
            active ? 'bg-[#E9F0FC] text-primario' : 'text-black',
            'group flex w-full items-center rounded-md px-2 py-2 text-sm',
          ]">
            <TrashIco class="mr-2 h-4 w-5 text-violet-400" aria-hidden="true" :color="active ? '#2166E5' : '#101010'" />
            Eliminar folio
          </button>
          </MenuItem>
        </div>
...
const desasignarFolio = async () => {
  const result = await get(refDB(db, `folios/${props.data.incidencia}/${props.data.folioKey}`));
      ...
      await remove(refDB(db, `foliosAsignados/${props.data.incidencia}/${result.val().tecnico}/activo`)).then(async () => {
        if (props.data.incidencia === "correctivos" && folioAsignado.hasChild("pendientes")) {
          await update(refDB(db, `foliosAsignados/${props.data.incidencia}/${result.val().tecnico}`), {
            ...
            await remove(refDB(db, `foliosAsignados/${props.data.incidencia}/${result.val().tecnico}/pendientes/${Object.keys(folioAsignado.val()["pendientes"])[0]}`))
          })
        }
      })
  ...
  await update(refDB(db, `folios/${props.data.incidencia}/${props.data.folioKey}`), { asignado: false })
  router.go(0);
}
...
const eliminarFolio = async () => {
  ...
  await remove(refDB(db, `folios/${props.data.incidencia}/${props.data.folioKey}`));
  router.go(0);
}
</script>
