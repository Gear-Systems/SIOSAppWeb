        ...
        <div v-if="props.incidencia == 1" class="mt-5 mr-10 ml-10 flex w-[50%]">
          ...
          >
            ...
            >
              <div
                ...
                class="absolute top-2 right-4 cursor-pointer font-bold text-red-700"
              >
                <XIcon class="h-5 w-5" />
              </div>
              <div class="flex h-full w-full items-center justify-center overflow-hidden">
                ...
                <div v-else class="flex h-full w-full justify-around p-1">
                  <img
                    :src="
                      fotos.antes.file64 ? fotos.antes.file64 : fotos.antes.file
                    "
          ...
          >
            ...
            >
              ...
              <div class="flex h-full w-full items-center justify-center object-contain overflow-hidden">
                <label
                  v-if="!fotos.durante.file"
                  for="foto-durante"
                  class="flex w-full cursor-pointer justify-around"
...
const emit = defineEmits(["setPaso"]);
...
onMounted(() => {
  ...
  if (props.data.imagenes) {
    if (props.data.imagenes.antes) {
      fotos.value.antes.file = props.data.imagenes.antes;
    }
    if (props.data.imagenes.durante) {
      fotos.value.durante.file = props.data.imagenes.durante;
    }
  }
});
...
const selectFileAntes = async (e) => {
  ...
  fotos.value.antes.file64 = URL.createObjectURL(e.target.files[0]);
};
const selectFileDurante = async (e) => {
  fotos.value.durante.file = e.target.files[0];
  fotos.value.durante.file64 = URL.createObjectURL(e.target.files[0]);
};
...
const actualizarEstado = async (imageUrl) => {
  ...
  if (parseInt(props.pasoDB) == parseInt(props.data.paso)) {
    object.paso = 3;
    object.status = "En proceso";
    object.estatusId = 4;
  }

  if (imageUrl.antes) {
    object.imagenes.antes = imageUrl.antes;
  } else {
    object.imagenes.antes = fotos.value.antes.file;
  }

  if (imageUrl.durante) {
    object.imagenes.durante = imageUrl.durante;
  } else {
    object.imagenes.durante = fotos.value.durante.file;
  }

  if (object.imagenes.antes != fotos.value.antes.file || object.imagenes.durante != fotos.value.durante.file || object.hasOwnProperty("paso")) {
    await update(
      child(refDB(db), `folios/preventivos/${route.params.id}`),
      object
    ).then(() => {
      router.go(0);
    });
