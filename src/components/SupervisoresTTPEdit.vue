<template>
  ...
  const actualizar = () => {
    ...
    if (newName) {
      get(child(refDB(db), `catalogo/supervisorTTP/${props.data.nombre}`)).then(
        (snapshot) => {
          update(
            refDB(db, `catalogo/supervisorTTP/${newName.value}`),
            snapshot.val()
          ).then((snapshot) => {
            if (props.data.nombre != newName.value) {
              remove(refDB(db, `catalogo/supervisorTTP/${props.data.nombre}`));
            }
            emit("closeModal");
          });
        }
