<template>
  <div
    class="absolute z-50 h-screen select-none rounded-tr-3xl rounded-br-3xl bg-white pt-10 shadow-xl transition-all duration-500 ease-in-out"
    :class="[$store.state.a.openSidebar == true ? 'w-[50%]' : 'w-0']"
  >
    <!-- Tarjeta bienvenido -->
    <div class="duration-600 flex items-center justify-center transition-all">
      <div class="absolute -right-7 h-7 w-7 rounded-full bg-black">
        <ChevronLeftIcon
          :class="[
    ...
    <!-- Separador -->
    <div
      class="duration-600 mt-12 flex w-full items-center justify-center transition-all"
    >
      <div class="flex w-[80%] border-b-[3px] border-black"></div>
    </div>
    <!-- Fin separador -->
    <div
      class="mt-8 flex justify-center overflow-hidden"
      :class="[$store.state.a.openSidebar ? 'w-full' : 'w-[100%]']"
    >
      <ul class="flex flex-col space-y-4">
        <li
          class="duration-600 flex cursor-pointer items-center justify-center space-x-4 rounded-md p-2 font-medium transition-all"
          :class="[
            $route.path == '/dashboard' ? 'bg-secundario text-primario' : '',
            $store.state.a.openSidebar ? 'w-full' : 'w-18',
          ]"
          @click="onRoute('/dashboard')"
        >
        ...
        <li
          class="duration-600 flex cursor-pointer items-center justify-center space-x-4 rounded-md p-2 font-medium transition-all"
          :class="[
            $route.path == '/capturar-folio'
              ? 'bg-secundario text-primario'
              : '',
            $store.state.a.openSidebar ? 'w-full' : 'w-18',
          ]"
          @click="onRoute('/capturar-folio')"
        >
          <div class="flex h-7 w-7 items-center justify-center rounded-full">
            <CapturarFolioIco
        ...
        <li
          class="duration-600 flex cursor-pointer items-center justify-center space-x-4 rounded-md p-2 font-medium transition-all"
          :class="[
            $route.path == '/consultar-folios'
              ? 'bg-secundario text-primario'
              : '',
          ]"
          @click="onRoute('/consultar-folios')"
        >
          <div class="flex h-7 w-7 items-center justify-center rounded-full">
            <ConsultarFolioIco
              :color="
                $route.path == '/consultar-folios' ? '#2166E5' : '#101010'
              "
            />
          </div>
          <TransitionRoot
        ...
        <li
          @click="onRoute('/reportes')"
          class="duration-600 flex cursor-pointer items-center justify-center space-x-4 rounded-md p-2 font-medium transition-all"
          :class="[
            $route.path == '/reportes' ? 'bg-secundario text-primario' : '',
          ]"
        >
          <div class="flex h-7 w-7 items-center justify-center rounded-full">
            <ReportesIco
    ...
    <div
      v-show="$store.state.a.openSidebar"
      :class="[
        'absolute bottom-0 left-1/2 -translate-x-1/2 -translate-y-1/2 transform transition-all delay-700',
        $store.state.a.openSidebar ? '' : 'hidden',
      ]"
    >
      v{{ version }}
    </div>
