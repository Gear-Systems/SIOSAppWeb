      ...
      <ul class="flex max-h-[220px] min-h-[220px] flex-col overflow-auto p-2">
        ...
        >
          ...
          <div class="basis-auto p-1">
            <DespachoTTPDropdownMenu :data="item" />
          </div>
        </li>
      </ul>
    </div>
...
import DespachoTTPDropdownMenu from "./DespachoTTPDropdownMenu.vue";

const query = ref("");
const props = defineProps(["despachoTTPData"]);

// Filtrar datos
const filteredData = computed(() =>
  query.value === ""
    ? props.despachoTTPData
    : props.despachoTTPData.filter((item) =>
