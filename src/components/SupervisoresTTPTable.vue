<template>
        ...
        >
          ...
          >
            ...
            <div class="basis-auto p-1">
              <SupervisoresTTPDropdownMenu :data="item" />
            </div>
          </li>
        </ul>
      </div>
  ...
  import FallasDropdownMenu from "./FallasDropdownMenu.vue";
import ConceptosDropdownMenu from "./ConceptosDropdownMenu.vue";
import SupervisoresTTPDropdownMenu from "./SupervisoresTTPDropdownMenu.vue";
  
  const query = ref("");
  const props = defineProps(["supervisorTTPData"]);
  
  // Filtrar datos
  const filteredData = computed(() =>
    query.value === ""
      ? props.supervisorTTPData
      : props.supervisorTTPData.filter((item) =>
          item.nombre
            .toLowerCase()
