<template>
  <div class="flex h-full w-full rounded-lg bg-[#F8FAFB] pb-12">
    <div class="mt-6 w-full shadow-md sm:rounded-lg overflow-hidden">
      <!-- Filtro -->
      <div
        class="lg:mx-16 flex items-center justify-between border-b-2 border-gray-300 bg-[#F8FAFB]"
      >
        <div
          v-if="false"
        ...
        <div class="flex basis-1/2 justify-between px-6 pr-28">
          ...
          <div class="relative">
            ...
            <input
              ...
              class="block lg:w-80 rounded-lg border border-gray-300 bg-gray-50 p-2 pl-10 text-sm placeholder:text-black focus:border-blue-500 focus:ring-blue-500"
      ...
      <!-- Tabla -->
      <div class="flex min-h-[200px] lg:justify-center bg-[#F8FAFB] overflow-auto">
        <table
          class="lg:mx-14 mt-8 h-fit text-left text-sm text-gray-500 dark:text-gray-400"
        >
          <tbody>
            <tr
            ...
            >
              ...
              <td class="py-4 px-12">
                {{
                  `${new Date(usuario.created).getDate()}/${new Date(
                    usuario.created
                  ).getMonth() + 1}/${new Date(usuario.created).getFullYear()}`
                }}
              </td>
              <td class="py-4 px-6">
              ...
              <td>
                <div class="mr-6">
                  <SwitchGroup>
                    <Switch
                      v-model="usuario.disabled"
                      :class="usuario.disabled ? 'bg-[#90B3F2]' : 'bg-primario'"
                      class="relative inline-flex h-[20px] w-[40px] cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75"
                    >
                      <span class="sr-only">Use setting</span>
                      <span
                        aria-hidden="true"
                        :class="
                          usuario.disabled ? 'translate-x-0' : 'translate-x-5'
                        "
      ...
      <div class="flex w-full justify-end lg:pr-20">
        <button
